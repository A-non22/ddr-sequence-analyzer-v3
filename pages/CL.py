...
def load_data():
    ...
    df.columns = [
        ...
        "Model Combinations", "Unused 1", "Unused 2"
    ]
    df = df.dropna(subset=["Start Point", "High / Low", "End Point Session"])
    return df
...
with col1:
    ...
    if not filtered1.empty:
        ...
        for _, row in outcome_counts1.sort_values("Count", ascending=False).iterrows():
            st.markdown(f"<div class='outcome-main'>{row['End Point Session']} | Count: {int(row['Count'])} ({row['Percentage']}%)</div>", unsafe_allow_html=True)

            session_subset = filtered1[filtered1["End Point Session"] == row["End Point Session"]]
...
with col2:
    ...
    if not filtered2.empty:
        ...
        for _, row in outcome_counts2.sort_values("Count", ascending=False).iterrows():
            ...
            session_subset = filtered2[filtered2["End Point Session"] == row["End Point Session"]]
            rdr_counts = session_subset["RDR Model"].value_counts()
            max_count = rdr_counts.max()
            top_rdrs = rdr_counts[rdr_counts == max_count]
            for i, (rdr, count) in enumerate(top_rdrs.items(), 1):
